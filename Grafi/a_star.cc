...
void rebuilt_path(int * pi, int t, int s){
  printf("\n====Rebuild path====\n");
  printf("pi array:\n");
  print_arr(pi);
...
void initialize_single_source(int* pi, int s, queue *priority, int *h, int t, int* d){
...
void a_star(Graph g, int s, int t){ // s is the source and t is the destination in the graph
  ...
  int* pi = new int[(SIZE+1)*sizeof(int)];
  int* d = new int[(SIZE+1)*sizeof(int)];
  int* h = new int[(SIZE+1)*(SIZE+1)*sizeof(int)];
  queue *priority = new queue();
  queue *S = new queue();
  int z = s;
  
  initialize_single_source(pi, s, priority, h, t, d);
  ...
  while(priority->is_empty()!=1){
	...
	if (g.reach[z*SIZE+u]!=0 && priority->value_already_present(u)==1){
	  if (d[u] > d[z]+g.reach[z*SIZE+u]){
	    d[u] = d[z]+g.reach[z*SIZE+u];
	    pi[u] = z;
	    priority->update_priority(u, d[u]+h[u*SIZE+z]);
	  }
	}
      }
    }
